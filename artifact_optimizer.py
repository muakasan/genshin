...
def perf_art_optim(dps_func, sandss, goblets, circlets, substats):
    ...
    BUDGET = 4*5 + GOOD_ROLLS*5 - 2 # initial + good rolls, you lose 2 to dead rolls
    ...
    for sands, goblet, circlet in itertools.product(sandss, goblets, circlets):
        ...
        roll_counts = [range(num_valid_arts[sub], num_valid_arts[sub] + GOOD_ROLLS*num_valid_arts[sub] + 1) for sub in substats]
        if count == 0:
            print(roll_counts)
        # mainstat_dist = {ms: artifact_mainstat(ms, star=5, lvl=20) for ms in [sandss, goblet, circlet]} # TODO does not account for multiple of the same main stat
        # hack to get around pyro type for now
...
def main():
    ...
    perf_art_optim(lambda mainstat_attr, substat_attr: n3cq_dps(dm_attr, mainstat_attr, substat_attr, artifact_set_effects, supress=True, ret_attr=True), sandss=[MainstatType.HP_PCT], goblets=["PYRO"], circlets=[MainstatType.CRIT_DMG, MainstatType.CRIT_RATE], 
